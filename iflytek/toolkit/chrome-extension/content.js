...
(function () {
    ...
    // 说明：禁用切屏检测的逻辑已迁移到 inject.js（运行在页面主世界 MAIN world）。
    // content.js 运行在扩展的隔离世界（Isolated World），无法修改页面真实的
    // window.fetch / EventTarget.prototype / document.hidden，因此这里不再处理切屏检测，
    // 避免误包裹隔离世界的 fetch 导致插件自身请求报 "Failed to fetch"。
