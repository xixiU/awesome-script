...
**描述**: 强大的推特/X多功能工具箱，集成屏蔽评论者、AI智能总结、AI评论过滤、启发式学习黑名单等功能。
...
🎯 **AI评论过滤功能**:
- 🤖 自动检测并隐藏低质量评论（广告、色情、无意义等）
- 📋 前置规则过滤（关键词黑名单、昵称前缀、用户简介等）
- 🎓 启发式学习：从拉黑历史自动提取常见模式（昵称/评论子串，≥5字符）
- 🔄 实时监听新加载评论并自动过滤
- 📊 可视化统计（拉黑、通过、AI拉黑比例）
- ⚙️ 规则管理：UI面板展示学习到的规则，支持启用/禁用/删除
- 📜 垂直滚动：规则列表超过高度时自动滚动，保持面板简洁
...
*AI评论过滤功能*:
1. 在配置面板启用"AI评论过滤"并配置API
2. 访问推文详情页，脚本自动过滤低质量评论
3. 启发式学习会从拉黑历史中自动提取≥5字符的常见模式
4. 在配置面板的"启发式黑名单"区域查看和管理学习到的规则

**注意事项**: 屏蔽操作不可撤销，AI功能需要配置有效的API Key。启发式学习需要至少10条拉黑历史才会开始工作。详细说明请参阅 `twitter/README.md`（[中文文档](twitter/README_zh.md)）。
