...
- ✅ 启用滑块验证码自动识别功能
- ✅ 启用"手动定位滑动验证码"菜单命令
- ✅ 支持自动检测和处理滑块验证码（需配合后端识别服务）

**说明**

- 滑块验证码功能需要配合后端识别服务使用
- 如果自动识别失败，可以使用"手动定位滑动验证码"菜单手动配置
- 手动配置步骤：点击 Tampermonkey 菜单 → "手动定位滑动验证码" → 依次点击大图片、小图片、滑块

### v4.3.0 (2026-05-08)

**通配符批量排除功能**

- ✅ 新增通配符支持：标记网站时可使用 `*` 和 `**` 批量排除页面
  - `*` 匹配单层路径（不包含 `/`）
  - `**` 匹配多层路径（包含 `/`）
- ✅ 优化交互体验：点击"标记当前网站没有验证码"时弹出输入框，可编辑网站地址
- ✅ 使用示例：
  - `x.com/user/status/*` - 排除该路径下所有页面
  - `x.com/**` - 排除整个域名
- ✅ 修复 `notice()` 函数 bug：解决消息提示显示两次的问题

**技术细节**

- 新增 `matchPattern()` 函数：将通配符模式转换为正则表达式进行匹配
- 新增全局黑名单列表 `captcha_blacklist_patterns`：存储通配符规则
- 保持向后兼容：精确匹配仍使用原有存储方式

### v4.2.9 (2026-05-06)

**提示优化 + 静默模式**

- ✅ 缩短识别成功提示显示时长：从默认 3000ms 降至 1500ms，减少视觉干扰
- ✅ 新增「静默模式」配置项：开启后识别成功仅静默填充，不弹出任何提示
- ✅ 静默模式可通过 Tampermonkey 菜单「⚙️ 打开 captchaConfig 配置」进行开关
