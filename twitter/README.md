...
- **触发**：每累积 20 条新拉黑记录学习一次；脚本启动时若有 ≥20 条积压立即补学
...
- **进度可见**：未达触发门槛时每次记录都会打印 `🎓 学习进度：已积累 N/20 条`，
  避免小批量拉黑时因毫无日志而被误判成学习失效
- 学到的规则可在配置面板查看、单条禁用或删除，也支持手动添加。规则增删改均原地生效不刷新页面
- 面板提供「🧹 清理重复规则」：不必等攒够 20 条新记录，随时对存量规则跑一遍规范化与合并去重
...
- 🧹 **修复启发式规则学不干净、堆积大量重复**：
  ...
  - 用实际的 49 条脏规则验证：压缩到 19 条，账号名规则全部清除，5 条正常评论零误伤、
    3 条垃圾评论仍全部命中
- ➕ **新增「🧹 清理重复规则」按钮**：学习需攒够 20 条新记录才触发，存量碎片不必等那么久。
...
- 📋 **打印学习进度**：小批量拉黑（如一次只拉 2 人）离 20 条触发门槛很远，此前没有任何学习相关
  日志，容易被误判成"学习坏了"。现在每次记录都会打印 `已积累 N/20 条`
