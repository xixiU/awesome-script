...
- 🐛 **修复时间线误过滤**：从推文详情页返回时间线（`/home`）时，AI 过滤的 MutationObserver 没有及时停掉，导致时间线上的推文被当成评论处理——已拉黑用户的推文被直接隐藏，其他推文还会被送给 AI 分类。现在路由切换时会立即 disconnect 评论监听器并清空 `blockedUsersSet`，MutationObserver 回调里也加了"只在推文详情页运行"的双保险
