...
- ⚡ **AI 过滤性能大幅提升**：
  - 批次上限从 20→30 条，减少 LLM 调用次数约 33%
  - 最多 2 个批次并行发送 LLM 请求，吞吐量翻倍
  - 去掉批次间 500ms 人为延迟
  - `maxTokens` 从 16384→2048，`temperature` 从 0.3→0.1，加速生成
  - block 操作改为 fire-and-forget，不阻塞 UI 标记和后续批次
  - MutationObserver debounce 从 1000ms→300ms，新评论更快响应
  - 初始启动延迟从 1500ms→800ms
- 🎯 **昵称匹配简介黑名单前缀**：昵称在 DOM 中直接可见，提前过滤掉昵称包含简介黑名单前缀的用户，无需发起 UserByScreenName API 请求，减少 API 调用
- 🛡️ **prompt 优化，减少误杀**：
  - 明确 username 不作为分类依据，只根据评论内容和昵称判断
  - 收紧 blacklist 标准：人身攻击限定为针对具体个人的直接死亡威胁，普通骂街不算；政治言论限定为煽动种族灭绝等极端暴力，普通讽刺体制不算
  - 新增总原则："宁可放过，不可误杀 — 如果评论虽然粗俗/擦边但明显是参与讨论，归为 normal"
...
- 🖥️ **UI 美化 - 宽屏模式**：新增"隐藏右侧栏"开关（默认开启），隐藏右侧推荐/趋势/搜索栏，推文内容从 600px 拉宽至 900px 平铺展开，左侧导航收缩到实际宽度，主内容区在可用空间内居中。可在配置面板关闭，刷新页面生效
