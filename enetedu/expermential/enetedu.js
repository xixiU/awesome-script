...
(function () {
    ...
    // ========== 网络负载优化：拦截 m3u8 视频 ts 文件 ==========
    // 通过将 TS 请求重定向到本地代理服务器来降低网络负载
    // targetWindow: 要注入拦截器的窗口对象
    // windowName: 窗口名称，用于日志输出
    function interceptVideoTsRequests(targetWindow = window, windowName = 'main') {
        console.log(`[视频拦截-${windowName}] 开始拦截 m3u8 视频 ts 文件请求...`);

        // 本地 TS 代理服务器地址
        const LOCAL_TS_PROXY = 'http://127.0.0.1:8888/ts/';

        let tsRequestCount = 0; // 统计拦截次数

        try {
            // 检查目标窗口是否有效
            if (!targetWindow || !targetWindow.XMLHttpRequest) {
                console.error(`[视频拦截-${windowName}] 目标窗口无效或不支持 XMLHttpRequest`);
                return;
            }
            ...
            targetWindow.XMLHttpRequest.prototype.open = function (method, url, ...args) {
                this._originalUrl = url;

                // 如果是 .ts 文件请求，重定向到本地代理服务器
                if (url && url.includes('.ts')) {
                    tsRequestCount++;
                    const fileName = url.substring(url.lastIndexOf('/') + 1);
                    const localUrl = LOCAL_TS_PROXY + fileName;

                    console.log(`[视频拦截-${windowName}] 拦截 TS 文件 #${tsRequestCount}: ${fileName}`);
                    console.log(`[视频拦截-${windowName}] 重定向: ${url} -> ${localUrl}`);

                    // 使用本地代理地址
                    return originalXHROpen.apply(this, [method, localUrl, ...args]);
                }
            ...
            if (targetWindow.fetch) {
                ...
                targetWindow.fetch = function (url, options) {
                    if (typeof url === 'string' && url.includes('.ts')) {
                        ...
                        // 重定向到本地代理服务器
                        return originalFetch.apply(this, [localUrl, options]);
                    }
            ...
            setInterval(() => {
                if (tsRequestCount > 0) {
                    console.log(`[视频拦截-${windowName}] 已拦截 ${tsRequestCount} 个 TS 文件，节省大量流量`);
                }
            }, 30000);

            console.log(`[视频拦截-${windowName}] 拦截器安装成功`);
        } catch (e) {
            console.error(`[视频拦截-${windowName}] 安装拦截器失败:`, e.message);
        }
    }

    // 初始化主窗口的视频 ts 拦截（在页面加载前就要拦截）
    interceptVideoTsRequests(window, 'main');
    ...
    function removeTabSwitchRestrictions() {
        ...
        const modifyReportingInterval = () => {
            // 只针对直播页面生效
            if (!utils.isLivePage()) {
                console.log('[Tab限制移除] 非直播页面，跳过上报间隔修改');
                return;
            }
                ...
                if (iframeElement) {
                    const enhanceIframeLogic = () => {
                        ...
                        if (iframeWindow) {
                            ...
                            // 在 iframe 中注入 ts 文件拦截器（阿里云播放器的请求是从 iframe 发起的）
                            interceptVideoTsRequests(iframeWindow, 'iframe');
            ...
            this.checkInterval = setInterval(() => {
                try {
                    ...
                    if (video) {
                        ...
                        if (video.currentTime > 0.92 * video.duration) {
