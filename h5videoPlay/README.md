...
- **精确控制**：逐帧前进/后退（D/E/G 键）
- **快速跳转**：5秒/20秒快进快退（方向键）
- **截图保存**：P 键一键截图
- **画中画**：I 键开启浮窗播放
- **全屏控制**：Enter 或 F 键切换全屏
- **原生进度条强制开启**：所有 H5 视频默认允许单击/拖动进度条跳转，穿透站点的 `controlsList`/`controls=false` 限制
...
| **↑ ↓** | 增加/减少音量 | |
| **Space** | 暂停/播放 | |
| **Enter** / **F** | 全屏 | Shift + Enter = 网页全屏 |
| **S** | 开启/关闭字幕翻译 🆕 | 仅支持同域视频 |
| **P** | 视频截图 | 自动保存 PNG |
| **I** | 画中画模式 | |
| **M** | 缓存视频 | |
| **N** | 下一集 | |
| **D** / **E/G** | 上一帧 / 下一帧 | YouTube 下一帧用 E |
...
- 所有 H5 视频默认启用原生 `controls`，单击/拖动进度条即可跳转
- 拦截站点对 `controlslist`、`disableRemotePlayback`、`disablePictureInPicture` 等禁用属性的反复设置
- MutationObserver 监听 DOM 与属性变化，防止播放器脚本事后关闭控件

**⌨️ 快捷键调整**

- 新增 **F 键切换全屏**（与 Enter 行为一致）
- 下一帧由 F 迁移至 **G 键**；YouTube 下一帧仍用 E 键
- 帮助文档精简为中文/英文双语（移除意大利语）
