...
**问题**：macOS 编译产物下载后因权限丢失导致无法打开；双击运行 EXE 时配置文件生成位置错误（生成的配置文件在临时目录中用户找不到）。

**解决手段**：

- **Artifact 打包**：GitHub Action 发布前自动将产物压缩为 `.zip`，保留文件权限（Executable Permission）。
- **智能配置路径**：`ConfigManager` 增加环境检测 (`sys.frozen`)，确保在打包模式下配置文件生成于**可执行文件同级目录**，而非临时解压目录。这样用户双击运行后即可在旁边看到 `model_config.json`。

#### 2. 日志与积压监控优化

**问题**：当 API 响应慢（如 10s+）时，队列积压警告日志高频刷屏，掩盖了关键的请求状态信息，让用户误以为程序卡死。

**解决手段**：

- **日志限流**：对“队列积压警告”增加频率限制（降至约 1s/次），防止日志爆炸。
- **状态透明化**：在调用大模型 API 前增加 `📡 发送请求...` 日志，明确区分“程序卡死”与“等待响应”。
